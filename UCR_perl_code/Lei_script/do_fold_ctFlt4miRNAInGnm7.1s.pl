...
use Getopt::Std;
getopts("hi:u:d:a:j:m:s:w:c:f:b:n:r:p:l:e:t:C:T:R:M:S:0:");
...
$h2p_set =~ s/\,/ /g;
	...
	printf STDERR ("\n M  %55s : %-25s","input soap M when use different Genome",$soap_M);
	printf STDERR ("\n S  %55s : %-25s\n\n","soap result file (-d VS -g)",$soap_result);
...
#		print STDERR "\t\tblastall -p blastn -F F -v 1000000 -b 1000000 -i $j -d $smallRNA -o $j.blastn -m 8\n";
	...
	#		print STDERR "\t\tsoap -D $j.index -M 0 -r 2 -v 0 -a $smallRNA -o $j.soap 2> $j.soap.err\n";
			if ($soap_M	== 0) {
				...
				system("soap -D $j.index -M 4 -r 2 -v 0 -a $smallRNA -o $j.soap 2> $j.soap.err");
...
#				system("cat $j.M0.soap $j.M1.soap $j.M2.soap > $j.soap");
		...
		#	$m	= $genome[$k2];	$m=~s/[\s|\t|\r|\n]+//g;
			...
			print STDERR "\t\tsoap done!";
	#		($m,$n,$h2p) =	sub_soap2histogram("$soap_result");	print STDERR "\tn=$n";#h2p=\n$h2p";
			($a,$spck,$m,$n,$h2p) =	sub_soap_check("$soap_result");	print STDERR "\tn=$n";	print STDERR "\tspck=$a\n";
...
#			}
			...
			$rci			= 0;	$rcount++;
			############################################# $rck:	name of miRNA
			############################################# $rcn:	num of miRNA, the nth structure
			############################################# $rci:	the nth line of ct structure
			############################################# $rcm:	the nth column of ct structure
		} else {
			$rcj	= $_;
			@rcbuf    = split(/\s+/,$rcj);
...
print STDERR "creat bbelong\t";
	my %bbelong=();	my $bk23	= 0;
	foreach $skey (keys %smrna) {
		$skey =~/^(\w+)\_(\d+)\_(\d+)/;	$bk23++;
		for ($bm = $2;$bm < $2+$3;$bm++) {
			$bbelong{$1}{$bm}{$skey}	= 1;
		}
	}
	print STDERR "$bk23\tLoad soap file\n";	$bk23	= 0;
...
#	}
	...
	#			print STDERR "\tMM!=0,",$_;
			...
			$bk23++;	#if ($bk23 % 10000 == 0) {print STDERR "$bk23,";}
			my %bbbmir	= ();	$bk4	= $7;
			for ($bm = $9; $bm < $9+$6 ;$bm += $6-1) {
				foreach $bk1 (keys %{$bbelong{$8}{$bm}}) {
					if (exists($bbbmir{$bk1})) {
						$bbbmir{$bk1}++;
					} else {
						$bbbmir{$bk1} = 1;
					}
			...
			$bn	= $6;	$bm	= $9;	$bk2	= $1;	### $bm,$9 start position;	$bn,$6 length;	$bk2,$1 $name of smallRNA
			foreach $bk1 (keys %bbbmir) {
				if ($bbbmir{$bk1}	< 2) {
					next;
				}
				if (exists($entry{$bk1})) {
		...
		#		$entry{$8}{$7}++;
				$bk1	=~  /(\d+)\_(\d+)$/;	#$bm	-= $1;
				$entry{$bk1}{$bk2}->[4]	= $bk4;			# +/-
				if ($smrna{$bk1}{"STRD"} eq "+") {
					$entry{$bk1}{$bk2}->[1]	= $bm-$1;			# start	############## the start position of match
					$entry{$bk1}{$bk2}->[2]	= $bm-$1+$bn-1;		# end	############## the end position of match
				} else {
					$entry{$bk1}{$bk2}->[1]	= $2 - ($bm-$1+$bn-1) + 1; #$bm -$1;			# start	############## the start position of match
					$entry{$bk1}{$bk2}->[2]	= $2 - ($bm-$1) + 1;		# end	############## the end position of match
				}
				if ($smrna{$bk1}{"STRD"}	eq $bk4) {
					if ($smrna{$bk1}{"STRD"} eq "+") {
						$entry{$bk1}{$bk2}->[5]	= $bm-$1;			# start	############## the start position of match
						$entry{$bk1}{$bk2}->[6]	= $bm-$1+$bn-1;		# end	############## the end position of match
					} else {
						$entry{$bk1}{$bk2}->[5]	= $2 - ($bm-$1+$bn-1) + 1; #$bm -$1;			# start	############## the start position of match
						$entry{$bk1}{$bk2}->[6]	= $2 - ($bm-$1) + 1;		# end	############## the end position of match
					}
				}
				$bk2	=~  /\_(\d+)x?$/;
				$entry{$bk1}{$bk4}	+= $1;
				if ($smrna{$bk1}{"STRD"}	eq $bk4) {
					if (exists($entry{$bk1}{"CN"})) {
						$entry{$bk1}{"CN"}	+= $1;
					} else {
						$entry{$bk1}{"CN"}	= $1;
					}
	...
	#			$entry{$bk1}{$bk2}->[0]	= $1;
	#			$entry{$bk1}{$1}->[3]	= $6;			# Len
	###################################################################### $bk1=$8:	name of miRNAs
	###################################################################### $bk2=$1:	name of small RNAs
...
#				$bk2	= $1;	$bk1	= $8;
	...
	#		print STDERR "Non-exist: $bk1,",$smrna{$bk1}{"RNA"},"-----------------2\n";
		...
		foreach $bk2 (sort keys %{$entry{$bk1}}) {
			if ($bk2 eq "CN" || $bk2 eq "+" || $bk2 eq "-") {
				next;
			}
#			if ($entry{$bk1}{$bk2}->[1] >= $smrna{$bk1}{"RS"}-$mismatch && $entry{$bk1}{$bk2}->[2] <= $smrna{$bk1}{"RE"}+$mismatch) {
			if ($entry{$bk1}{$bk2}->[5] >= $smrna{$bk1}{"RS"} && $entry{$bk1}{$bk2}->[6] <= $smrna{$bk1}{"RE"}) {
				$sm += $entry{$bk1}{$bk2}->[0];	#$sm = $sm > $entry{$bk1}{$bk2}->[0] ? $sm : $entry{$bk1}{$bk2}->[0];
			}
			if ($entry{$bk1}{$bk2}->[5] >= $smrna{$bk1}{"MS"} && $entry{$bk1}{$bk2}->[6] <= $smrna{$bk1}{"ME"}) {
				$sn += $entry{$bk1}{$bk2}->[0];
			}
		}			if($entry{$bk1}{"CN"} == 0){print "name=$bk1, CN=",$entry{$bk1}{"CN"},"\n";}
...
#	undef(%entry);#=();
	my $bbbm	= $bm;	my $bbbop	= $bop;
########################################### output the histogram file and check the +/- strand bias #######################################################
	$bk1	= 0;	$bflag	= 0;	$ba = 0;	$bm = 0;	$bn	= 0;	$bop = "";
	foreach $bk1 (sort keys %entry) {	
#		print STDERR "\n\tS $bk1:\t+:",$entry{$bk1}{"+"},"\t\*$multiple4strand:",$multiple4strand*$entry{$bk1}{"+"},"\t-:",$entry{$bk1}{"-"},"";
		...
		$ba = 0;	
		foreach $bk2 (sort keys %{$entry{$bk1}}) {
			if ($bk2 eq "+" || $bk2 eq "-" || $bk2 eq "CN") {
				next;
			}
			if ($entry{$bk1}{$bk2}->[4] ne $bflag) {
				next;
			}
			$bk2	=~  /\_(\d+)x?$/;
			$bk3	= $1;
...
#		#		$entry{$8}{$7}++;
#				$entry{$bk1}{$bk2}->[0]	= $bk4;			# +/-
#				$entry{$bk1}{$bk2}->[1]	= $bm;			# start	############## the start position of match
#				$entry{$bk1}{$bk2}->[2]	= $bm+$bn-1;		# end	############## the end position of match
#	#			$entry{$bk1}{$bk2}->[3]	= $6;			# Len
#	###################################################################### $bk1=$8:	name of miRNAs
#	###################################################################### $bk2=$1:	name of small RNAs
#
#				$bk2	=~  /\_(\d+)x?$/;
