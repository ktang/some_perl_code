...
sub sub_strucCheck
{
	...
	#	$ct{"title"}	=~ /(\d+)\,(\d+)\s*$/;
	#	$ovlp_s	= $1;	$ovlp_e	= $2;	#print STDERR $ct{"title"},"\ts=$ovlp_s\te=$ovlp_e\n";
		...
		$sline1	= "";	#$opRNA	= "";
		for ($sk1 = 0;$sk1 <= @sbuf - $winLen ;$sk1+=$slideLen) {
			splice(@stmp,0);	splice(@stmp1,0);	$opRNA	= "";
			for ($sk2 = $sk1;$sk2 < $sk1+$winLen ;$sk2++) {
				$stmp[$sk2 - $sk1]	= $sbuf[$sk2];
			}

			#----------------------------------------------------mismatch---------------------------------
			$sm	= 0;
			for ($sk2 = 0;$sk2 < $winLen ;$sk2++) {
				if ($stmp[$sk2] == 0) {
					$sm++;
				}
				$stmp1[$sk2]	= $stmp[$sk2];
	...
	#		print STDERR "mismatch=$sm, ";
			...
			if ( $sm - $sn < $winLen - $mismatch) {
				next;
			}
			if ($sm - $sn > $winLen + $mismatch) {
				next;
			}
			if ($sm <= $sk1 || $sn >= $sk1+$winLen) {
			} else {
				next;
			}
			#----------------------------------------------------bulges---------------------------------
			...
			#----------------------------------------------------bulges---------------------------------
	#		$sm	= sub_overlap($sk1+1,$sk1+$winLen,$stmp1[0],$stmp1[@stmp1-1],$ovlp_s,$ovlp_e,$overlap);
	#		if ($sm == 0) {
	#			next;
	#		}
			$sline1	.= ($sk1+1). ".." . ($sk1+$winLen);
			$opRNA	.= 	">".$ct{$skey}{"name"}."_" . ($sk1+1). ".." . ($sk1+$winLen) . "\n";
			for ($sm = $sk1;$sm < $sk1+$winLen ;$sm++) {
				$opRNA	.= $ct{$skey}{$sm}{1};
			}
			$opRNA	.= "\n";
