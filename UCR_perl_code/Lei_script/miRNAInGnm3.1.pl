...
use Getopt::Std;
getopts("hi:g:d:l:p:o:q:c:e:t:0:");
...
my $ctFlt		= "do_fold_ctFlt4miRNAInGnm1.1.pl -0 0";#4miRNAInGnm
...
while ($flag0) {
	...
	printf STDERR ("\n d  %45s : %-25s","input small RNA database (formatdb)",$smallRNA);
...
#	next;
	...
	system("perl $path/$qsub $qsub_set -p \"perl $path/$ctFlt\" -i \"-i\" -m \"$fold_ctFlt -p $path -d $smallRNA -t $b2h_set $h2p_set\" -q $foldFile -o \"2> $foldFileOut\" 2> $foldFile.err");	# --mfold=*,*,5 
	$a	= -1;
	while ($a == -1) {
...
#	system("perl $path/$searchSeq -i $foldFile -l $foldFile.ctFlt.txt > $foldFile.ctFlt.fa 2> $foldFile.err");
	...
	system("cat $foldFile.ctFlt.hist >> $infile.miRiG.ctFlt.hist");
...
#	print "$0 -i blast_m8.blastn -t 90\n";
	print "$0 -i blast_m8.blastn -g genome.fa -d smallRNA.fa -l $len -p $path -o $output -q $qsub_set -c $fold_ctFlt\n";
	print"\n============================================================\n\n";
