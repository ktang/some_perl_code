...
# Version: 6.0s use soap2p to filter h2p
#**************************
...
use Getopt::Std;
getopts("hi:g:d:l:p:o:s:q:c:a:j:m:f:e:t:k:0:");
...
my $ctFlt		= "do_fold_ctFlt4miRNAInGnm4.0s.pl -0 0";#4miRNAInGnm
...
$k3	= 0;	my $k6	= 0;	my $k10	= 0;	my $k11	= 0;	my	$k12	= 0;
for (;$k1 <= $i ;$k1++) {
	$j	= $gnmName[$k1];	$k12++;
	if ($k11 > $max_seq) {
		%seg=();	$k11	= 0;	$k12	= 1;
...
#		foreach $n (sort numerically keys %{$gnm{$j}{$m}} ) {		# n	= end
			...
			for ($k2 = $minLen; $k2 <= $b ;$k2+=$addLen) {
				...
				if (exists($seg{$key})) {
					...
					$seg{$key}->[0]	= $line.";$m,$n;".($m-$a).",".($n-$a);
					$seg{$key}->[1]	= substr($genome[$k1],$a,$k2);	$k3++;	$k11++;
				}
