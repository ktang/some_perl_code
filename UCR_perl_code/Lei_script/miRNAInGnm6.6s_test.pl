...
use Getopt::Std;
getopts("hi:g:d:l:p:o:s:q:c:a:j:m:f:e:t:k:G:0:");
...
$k3	= 0;	my $k6	= 0;	my $k10	= 0;	my $k11	= 0;	my	$k12	= 0;	my $cpn1= 0;	#my $cpn2=0;
for (;$k1 <= $i ;$k1++) {
	$j	= $gnmName[$k1];	$k12++;
	if ($k11 > $max_seq) {
		%seg=();	$k11	= 0;	$k12	= 1;
...
#	undef(%strand);	my %strand=();
	...
	$a	= -1;
	do {
		system("perl $path/$qsub $qsub_set -s $splitQuery -p \"perl $path/$ctFlt\" -i \"-i\" -m \"$fold_ctFlt -p $path -j $blastORsoap -d $smallRNA -t $b2h_set -e $h2p_set\" -q $foldFile -o \"2> $foldFileOut\" 2> $foldFile.err");	# --mfold=*,*,5 
		$a	= -1;
		while ($a == -1) {
...
#	print STDERR "\tperl $path/searchseq1.4s.pl\n";
	...
	if ($debug	!= 0) {
		system("less $foldFile.ctFlt.pkup.fa | gawk '{if(\$_~/^>/){gsub(\">\",\"\",\$1);print \"__DEBUG__PickUp__\t\"\$1}}'");
	}	
