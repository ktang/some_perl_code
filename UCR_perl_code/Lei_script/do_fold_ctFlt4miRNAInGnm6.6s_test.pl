...
use Getopt::Std;
getopts("hi:u:d:a:j:m:s:w:c:f:b:n:r:p:l:e:t:C:T:R:M:g:0:");
...
my $debug		= (defined $opt_g) ? $opt_g : 0;	# 0: donot oouput; 1: output debug info 
...
#		system("rm $j*");
	...
	#		print STDERR $ct{$key2}{$key}{"FE"},"=FE,\n";
			if ($ct{$key2}{$key}{"FE"} > $maxFree) {
				if ($debug != 0 ) {
					print STDERR "__DEBUG__FreeEnergy__\t",$ct{$key2}{$key}{"name"},"\t",$ct{$key2}{$key}{"FE"},"\t$key2\t$key\n";
				}
				delete($ct{$key2}{$key});
				$ct{$key2}{"num"}--;
				$ii++;
...
if ($debug != 0) {
	system("perl $path/$searchSeq -i $infile.ctFlt.hist_p -l $infile.ctFlt.spck > $infile.ctFlt.hist 2>> h2p.err.h2p");
	system("cat $infile.ctFlt.hist_p >> h2p.out.h2p_p");
	system("perl $path/$hist2predict  $h2p_set -i $infile.ctFlt.hist -c $infile.ctFlt.txt > $infile.ctFlt.h2p 2>> h2p.err.h2p");
} else {
...
system("cat $infile.ctFlt.h2p >> h2p.out.h2p");
...
#				if ($debug != 0 ) {
#					print STDERR "__DEBUG__MISMATCH__\t$skey2\t$skey\t",$sm,"\n",
#				}
				next;
			}
			#----------------------------------------------------order---------------------------------
	...
	#		}
		...
		#	print STDERR "ct=$sct\n";
			...
			if ($debug != 0 ) {
				print STDERR "__DEBUG__Structure__\t",$ct{$key2}{$key}{"name"},"\t$skey2\t$skey\n",
			}
		}
	}
}
...
#			if ($entry{$bk1}{$bk2}->[1] >= $smrna{$bk1}{"RS"}-$mismatch && $entry{$bk1}{$bk2}->[2] <= $smrna{$bk1}{"RE"}+$mismatch) {
		...
		if (1.0*($sn+$sm) / $entry{$bk1}{"CN"} >= $minRatio) {
			...
			if ($debug != 0 ) {
				print STDERR "__DEBUG__PercentOfmiRNA__\t$bk1\t",1.0*($sn+$sm) / $entry{$bk1}{"CN"},"\n";
			}
		}
	}
	undef(%entry);#=();
...
#		print STDERR "\n\tS $bk1:\t+:",$entry{$bk1}{"+"},"\t\*$multiple4strand:",$multiple4strand*$entry{$bk1}{"+"},"\t-:",$entry{$bk1}{"-"},"";
		if ($entry{$bk1}{"+"} >= $multiple4strand*$entry{$bk1}{"-"}) {
			...
			if ($debug != 0 ) {
				print STDERR "__DEBUG__StrandBias__\t$bk1\t",$entry{$bk1}{"+"},":",$entry{$bk1}{"-"},"\n";
			}
			next;
		}
