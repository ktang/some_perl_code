  ...
  while($line = <$IN2>)
  {
    ...
    if (exists $genes{ $parts[8]})
    {
    	...
    	else
    	{
    		...
    		for($i = $parts[10]; $i < $parts[10] + $parts[6]; $i++ )
    		{
    			$gene_depth{ $parts[8]} -> [$i] += $parts[1];
    		}
    	}
    }	
  }
  ...
  foreach my $gene(sort keys %genes)
  {
 	 print $OUT $gene,"\t",$genes{$gene},"\t";
		...
		for($i = 0; $i < $genes{$gene} -1; $i++)
		{
		 ...
		 if(!defined($gene_depth{$gene}->[$genes{$gene} -1]))
		  {
			$gene_depth{$gene}->[$genes{$gene} -1] = 0;
	   	  }
		  print $OUT $gene_depth{$gene}->[$genes{$gene} -1],"\n";
