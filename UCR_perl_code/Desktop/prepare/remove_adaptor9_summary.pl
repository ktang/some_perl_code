...
open ($SUM, '>', $out_sum)
  or die  "Cannot open:$out_sum:$!";
    print $SUM  "Number of seqs that are trimmed by different length:\n";
    print $SUM join("\t", (">=9", "8", "7", "6", "5", "4", "3", "2", "1", "0")), "\n";
    print $SUM join("\t", ($trim_9_over, $trim_8, $trim_7, $trim_6, $trim_5, $trim_4, $trim_3, $trim_2, $trim_1, $no_trim)), "\n";
    print $SUM "Number of short seqs (< $min_sRNA_len nt): $no_short_seqs", " Number of good seqs: $no_good_seqs\n";
	print $SUM "Number of seqs contains 'N': $no_seqs_has_N\n";
	print $SUM ">= 9nt & Y: $no_seqs_ge9 ", ">=9nt & N: $no_seqs_ge9_N\n";
	print $SUM "<9nt & Y: $no_seqs_ls9 ", "<9nt & N: $no_seqs_ls9_N\n";
	print $SUM "no adaptor & Y: $no_seqs_no ", "no adaptor & N: $no_seqs_no_N\n";
close($SUM);
