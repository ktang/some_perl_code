...
NOTECE: the output of TAMALg should be sorted.

this script is used to select the
peaks which the trim mean is 

only 

greater
than the cutoff,output the trimmean.
=cut
...
for (my $i = 0; $i <= $#beds; $i++)
{
	...
	{
			$num_less_four++;
			print STDERR "$this\t$#array1\t";
			print STDERR join "\t",@array1;
			print STDERR "\n";
	}
	else
	{
