...
=head2
used in Purdue (Feb27);

count CG only
#chr	.	.	start	end	total_CG	mCG_rdd	mCG_col	diff_percentage
...
my $ccker_col0 = "/Users/tang58/Nimblegen_project/Ecker_data/mc_col0_tair9_no_warn.gff";

my $ccker_rdd = "/Users/tang58/Nimblegen_project/Ecker_data/mc_rdd_tair9_no_warn.gff";
...
LOOP: while ( $b <= $#beds and $c <= $#col0s )
{
	...
	if ( lc($pts_bed[0]) eq lc($pts_C[0]))
	{
		...
		elsif ( ( $pos_C >= $start) and ($pos_C <= $cnd))
		{
			#if ( $pts_C[7] > 0 )
			#{
					if ($pts_C[5] eq "CG") { $col_met_CG{$chr}->{$pts_bed[1]}++ }
...
LOOP: while ( $b <= $#beds and $c <= $#rdds )
{
	...
	if ( lc($pts_bed[0]) eq lc($pts_C[0]))
	{
		...
		elsif ( ( $pos_C >= $start) and ($pos_C <= $cnd))
		{
			#if ( $pts_C[7] > 0 )
			#{
				    	if ($pts_C[5] eq "CG") { $rdd_met_CG{$chr}->{$pts_bed[1]}++ }
...
for (my $i = 0; $i <= $#beds; $i++)
{
	...
	if ($c == 0)
	{
		...
		{
				print STDERR "$chr\t.\t.\t$pts[1]\t$pts[2]\t$c\t$mC_rdd\t$mC_col0\t$diff\n";
				die;
		}
	}
	...
	print "$chr\t.\t.\t$pts[1]\t$pts[2]\t$c\t$mC_rdd\t$mC_col0\t$diff\n";
#	if ( ($i < 10) or ($i > ($#beds -5)))
	#{
