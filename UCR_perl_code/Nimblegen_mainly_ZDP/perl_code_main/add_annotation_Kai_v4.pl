...
use bed_trimmean(4 col) input, output trimmean.
=cut
...
my $func = "/Users/kaitang/Desktop/TAIR/TAIR9_functional_descriptions";
open(FF, $func) or die "Can't open $func: $!";

my $gene_TE = "/Users/kaitang/Desktop/TAIR/TAIR9_GFF3_genes_transposons.gff";
open(GFF, $gene_TE) or die "Can't open $gene_TE:$!";

my $TE_frag = "/Users/kaitang/Desktop/TAIR/TAIR9_Transposable_Elements.txt";
open(TF, $TE_frag) or die "Can't open $TE_frag:$!";

my $inter_file = "/Users/kaitang/Desktop/TAIR/TAIR9_intergenic_20090619";
...
while(<MT>){
	...
	next unless(/\w+/);
	chomp;
	my ($chr, $start, $end,$trimmean) = split /\t/;
	...
	$mat_regions{$chr}->{$start} = [$start, $end, $trimmean];
}
...
foreach my $chr(sort keys %mat_regions){
    ...
    foreach my $start(sort keys %reg){
		...
		print join("\t", ($chr, $start, $end, $trimmean,$gene, $geneType, $geneAnnot,
		     $TE, $TEFamily, $inter)), "\n";
	}
}			
