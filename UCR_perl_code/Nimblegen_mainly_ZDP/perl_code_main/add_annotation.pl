...
while(<MT>){
	...
	$mat_regions{$chr}->{$start} = [$start, $end, $p];
}
...
while(<GFF>){
	...
	if($temp[2] eq "gene" || $temp[2] eq "pseudogene" || $temp[2] eq "transposable_element_gene"){
		foreach my $st(keys %{$mat_regions{$chr}}){
			my ($start, $end, $p) = @{$mat_regions{$chr}->{$st}};
