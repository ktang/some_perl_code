...
use bed(with trimmean in the laso column) file as input

output format should be
chr	start	end	length	trimmean	total_C	mC	percentage
...
my $usage = "$0 <peak_bed_file> <Ecker_data> ";
...
for (my $i = 0; $i <= $#beds; $i++)
{
	...
	print "$chr\t$pts[1]\t$pts[2]\t$pts[3]\t$len\t$c\t$met_C\t$percentage\n";
	#if ( ($i < 10) or ($i > ($#beds -5)))
	#{
