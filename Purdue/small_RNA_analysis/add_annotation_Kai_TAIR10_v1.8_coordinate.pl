...
#v1.8 coordinate
#input format change from bed to 1:1-4
...
my $usage = "$0 < coordinate_file_withHead> STDOUT";
...
open(MT, $infile) or die "Can't open $infile:$!";
...
#	}
	my @a = split "\t";
	if( $a[0] =~ /(\S+):(\d+)-(\d+)/){
		die $_ unless (defined $chr_symbol{$1}  ) ;
		my $chr = $chr_symbol{$1} ;
		my $start = $2;
		my $end = $3;
...
print join("\t", ($infile_head,"Gene", "Strand","GeneType", "GeneAnnot",
