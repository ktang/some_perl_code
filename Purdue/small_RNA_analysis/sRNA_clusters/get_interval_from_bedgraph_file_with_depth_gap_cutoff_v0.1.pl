...
my ($last_chr, $last_start, $last_end, $last_max_depth) = ("chr0", -1 , -1, -1);
...
while(<>){
	...
	if($chr ne $last_chr ){
		if( $last_chr ne "chr0"){
			print join("\t",($last_chr, $last_start, $last_end, $last_max_depth)), "\n";
		}
		
		#($last_chr, $last_start, $last_end)  =  ($chr, $pos, $pos);
		($last_chr, $last_start, $last_end, $last_max_depth)  =  ($chr,  $start_l, $end_l, $dep ) ;
