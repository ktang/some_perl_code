...
my $script = "/Users/tang58/scripts_all/perl_code/Purdue/small_RNA_analysis/1_trim_adaptor/sRNA_trim_adapter_v0.1.pl";
...
my $usage = "$0 \n <indir> <outdir> <ada_seq> <trim5_bp>\n\n";
die $usage unless(@ARGV == 4);

my $indir = shift or die;
my $outdir = shift or die;
my $ada_seq = shift or die;
my $trim5_bp = shift ;
...
foreach my $file (@files){
	...
	if ( $file =~ /(\S+)\.f.*q\.gz/) {
		...
		my $cmd = "$script $ada_seq fq_gz $input $outdir $pre $trim5_bp";
		print STDERR $cmd, "\n\n";
		if (!$debug) {
			`$cmd`;
		}
