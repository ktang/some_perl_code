...
all C
fold means
 (rdd_mC_plus + rdd_mC_minus - Col_mC_plus - Col_mC_minus)/(Col_mC_plus + Col_mC_minus)
 
 if (Col_mC_plus + Col_mC_minus) == 0, rdd_mC_plus + rdd_mC_minus should >= 2
...
$counts{$peak_chr}->{$pts_peak[1]} = ($peak_seq =~ s/CG/CG/g);
}
...
=head
	if ($pts[4] eq "+"){
			print join("\t", ($pts[0], $pts[1], $pts[2], $pts[4], $C, $mC_rdd, $mC_rdd_score, $mC_col0, $mC_col0_score)), "\n";

	}elsif ($pts[4] eq "-"){
			print join("\t", ($pts[0], $pts[1], $pts[2], $pts[4], $C_minus, $mC_rdd_minus, $mC_rdd_minus_score, $mC_col0_minus, $mC_col0_minus_score)), "\n";

	}elsif ($pts[4] eq "NONE"){
		my $mC_rdd_ave = ($mC_rdd + $mC_rdd_minus)/2;
		my $mC_Col0_ave = ($mC_col0 + $mC_col0_minus)/2;
		my $mC_rdd_score_ave = ($mC_rdd_score + $mC_rdd_minus_score)/2;
		my $mC_Col0_score_ave = ($mC_col0_score + $mC_col0_minus_score)/2;
		print join("\t", ($pts[0], $pts[1], $pts[2], $pts[4], $C, $mC_rdd_ave, $mC_rdd_score_ave, $mC_Col0_ave, $mC_Col0_score_ave)), "\n";

	}else{
		print STDERR "wrong for strands: $this\n";	
	}
=cut	
