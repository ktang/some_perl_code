...
#my $wig_file = $ARGV[0];
#die "wig_file " unless (-e $wig_file);
#my %mCs;
#read_wig($wig_file, \%mCs);
...
foreach my $input (@files){
	...
	my %mCs;
	read_wig("$indir/$input", \%mCs);
	my @num_array = (0, 0, 0, 0, 0, 0, 0);
	
	foreach my $chr (sort keys %mCs){
		foreach my $pos(sort {$a <=> $b} keys %{$mCs{$chr}}){
