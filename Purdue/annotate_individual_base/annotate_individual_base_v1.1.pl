...
# output all the hyper-DMP distribution
...
my %idm1_real_pos;
foreach my $chr(keys %pos_idm1_hypers){
	foreach my $pos (keys %{$pos_idm1_hypers{$chr}}){
		$idm1_real_pos{$chr}->{$pos} = 1;
	}
}
...
print join("\t", (0, "upstream_1000bp", "5'_UTR", "CDS", "intron", "3'_UTR", "downstream_1000bp")),"\n";
