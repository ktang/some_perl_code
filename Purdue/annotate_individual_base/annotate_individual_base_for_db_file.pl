...
# this script annotate_individual_base_for_db_file.pl
# take the db_file as input, output for 6 class(hyper,hypo CG,CHG, CHH)
# number in exon, intron, UTR, transposon and ...
...
my $db_file = shift;
die "df_file" unless (-e $db_file);

# includeing all alternative splicing RNA
...
# read_file	my ($file, $ref_hyper, $ref_hypo) = @_;
	...
	read_file($db_file, \%pos_hypers, \%pos_hypos);
...
my $TE_file = "/Users/tang58/DataBase/TAIR10/TE/TAIR10_GFF3_transposons.gff";
#1:up
#2:5'_UTR
#3:CDS
#4:intron
...
open(TE, $TE_file) or die "TE";
while(<TE>){
	$num_debug ++;
	chomp;
	my @a = split "\t";
	my $chr = lc $a[0];
	my ($start, $end) = ($a[3], $a[4]);
	for my $i($start..$end){
		$types{$chr}->[$i] = 7;
	}
}
close(TE);
print STDERR "TE: " , $num_debug, "\n";
...
print STDERR "CG_hyper:";
count(\%{$pos_hypers{CG}});

print STDERR "CHG_hyper:";
count(\%{$pos_hypers{CHG}});

print  STDERR "CHH_hyper:";
count(\%{$pos_hypers{CHH}});

print STDERR  "CG_hypo:";
count(\%{$pos_hypos{CG}});

print  STDERR "CHG_hypo:";
count(\%{$pos_hypos{CHG}});

print STDERR  "CHH_hypo:";
count(\%{$pos_hypos{CHH}});
...
sub count{
	...
	print  STDERR  "total_num: $sum\n";

	my @pers = map { sprintf ("%.1f", 100 * $_ / $sum)."%" } @num_array;

	print  STDERR join("\t", ("intergenic", "upstream_1000bp", "5'_UTR", "CDS", "intron", "3'_UTR", "downstream_1000bp", "TE")),"\n";
	print STDERR  join("\t", @num_array), "\n";
	print STDERR  join("\t", @pers), "\n\n";
}
	...
	while(<IN>){
		...
		if($sum > 0){
			if (  (   ($a[8] * $a[7] + $a[11] * $a[10]) / $sum ) > $a[5] ){
				$ref_hyper->{$type}->{$a[0]}->{$a[1]} = 1;
			}
			else{
				$ref_hypo->{$type}->{$a[0]}->{$a[1]} = 1;
			}
		}
		else{
			$ref_hypo->{$type}->{$a[0]}->{$a[1]} = 1;
