...
#v1.2 annotate all C;
#this script distinguish CG, CHG and CHH
use strict;
my $debug = 0;
if($debug){
...
my @num_array_CG = (0, 0, 0, 0, 0, 0, 0);
my @num_array_CHG = (0, 0, 0, 0, 0, 0, 0);
my @num_array_CHH = (0, 0, 0, 0, 0, 0, 0);
...
output(\@num_array_CG, "CG");
output(\@num_array_CHG, "CHG");
output(\@num_array_CHH, "CHH");
exit;
...
sub output{
	...
	print $type, "_total_num: $sum\n";

	my @pers = map {sprintf ("%.2f", 100 * $_ / $sum) } @{$ref};
