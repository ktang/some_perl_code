...
my $P1_adaptor_1st_MAXnt = substr($adaptor_P1, 0, 9);
...
my $P1_adaptor_1st_MAXnt_rev = reverse $P1_adaptor_1st_MAXnt;
...
my ($P1_trim_1,$P1_trim_2, $P1_trim_3, $P1_trim_4, $P1_trim_5, $P1_trim_6, $P1_trim_7,
	$P1_trim_8, $P1_trim_MAX_over, $P1_no_trim) = (0) x 10; # number of seqs subject to different trim
...
my $P2_adaptor_1st_MAXnt = substr($adaptor_P2, 0, 9);
...
my $P2_adaptor_1st_MAXnt_rev = reverse $P2_adaptor_1st_MAXnt;
...
my ($P2_trim_1,$P2_trim_2, $P2_trim_3, $P2_trim_4, $P2_trim_5, $P2_trim_6, $P2_trim_7,
	$P2_trim_8, $P2_trim_MAX_over, $P2_no_trim) = (0) x 10; # number of seqs subject to different trim
...
my $output_P1 = $output_pre."_reads1.fastq"; 
my $output_P2 = $output_pre."_reads2.fastq"; 

my $output_S1 = $output_pre."_mates1.fastq"; 
my $output_S2 = $output_pre."_mates2.fastq"; 
...
###################################################################################
	...
	#	for( my $i = 0; $i < $original_length; $i++ ){
		for( my $i = 0; $i < $P2_original_length; $i++ ){
		...
		if( !$P2_cutoff_hit ){
			...
			$P2_best_length = $P2_original_length;
		}
...
##########################################################################################################
	...
	#	if($P1_seqstr =~ /(\w*$P1_adaptor_1st_13nt)\w*/){	
		if($P1_seqstr =~ /(\w*$P1_adaptor_1st_MAXnt)\w*/){	
	...
	#		if ($P1_new_seqstr =~ /\w*($P1_adaptor_1st_13nt_rev\w*)/){
			if ($P1_new_seqstr =~ /\w*($P1_adaptor_1st_MAXnt_rev\w*)/){
				$P1_new_seqstr = $1;
			}
			$P1_new_seqstr = reverse $P1_new_seqstr;
	#		$P1_new_seqstr =~ s/$P1_adaptor_1st_13nt//;
			$P1_new_seqstr =~ s/$P1_adaptor_1st_MAXnt//;
...
##########################################################################################################
	...
	if(length($P2_seqstr) < $minLen){
		...
		if($P2_seqstr =~ /(\w*$P2_adaptor_1st_MAXnt)\w*/){			
			$P2_new_seqstr = $1;

			$P2_trim_MAX_over++;

			$P2_new_seqstr = reverse $P2_new_seqstr;
			if ($P2_new_seqstr =~ /\w*($P2_adaptor_1st_MAXnt_rev\w*)/){
				$P2_new_seqstr = $1;
			}
			$P2_new_seqstr = reverse $P2_new_seqstr;
			$P2_new_seqstr =~ s/$P2_adaptor_1st_MAXnt//;
...
sub summary{
	...
	print join("\t",($P1_trim_MAX_over, $P1_trim_8,
...
#	print join("\t",($P2_trim_13_over,$P2_trim_12,$P2_trim_11,$P2_trim_10,$P2_trim_9, $P2_trim_8,
	print join("\t",($P2_trim_MAX_over, $P2_trim_8,
