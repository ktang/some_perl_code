...
foreach my $file (@files){
	if($file =~ /(\S+)_bed/) {
		my $output = $1."_our_".$label."_minCov_notScore".$min_coverage ."_minScore10_2fold_methylation_info.txt";
		...
		chomp $head;
		#my $h = "mC_Our".$label."_";
		print OUT join("\t", ($head, "mut_0_1kb_num", "WT_0_1kb_num","mut_0_1kb_score", "WT_0_1kb_score", "Hyper_0_1kb", "mut_1_2kb_num", "WT_1_2kb_num","mut_1_2kb_score", "WT_1_2kb_score", "Hyper_1_2kb")), "\n";
		...
		for(my $i = 1; $i <= $#ins; $i++){
			...
			my ($num_mut_1kb, $num_mut_2kb, $num_mut_total, $num_wt_1kb, $num_wt_2kb, $num_wt_total ) = (0, 0, 0, 0, 0, 0);
			...
			if ($a[4] eq "+"){
					...
					for (my $j = $start; $j <= $end; $j++){
						if(defined $muts{$chr}->[$j]){$mC_ros3_1kb += abs($muts{$chr}->[$j]) ; $num_mut_1kb ++}
						if(defined $cols{$chr}->[$j]) {$mC_col0_1kb += abs($cols{$chr}->[$j]); $num_wt_1kb ++}
					}
					
					my ($new_start, $new_end) = ( $start - 1000 , $start - 1);
					for (my $j = $new_start; $j <= $new_end; $j++){
						if(defined $muts{$chr}->[$j]){$mC_ros3_2kb += abs($muts{$chr}->[$j]); $num_mut_2kb ++ }
						if(defined $cols{$chr}->[$j]) {$mC_col0_2kb += abs($cols{$chr}->[$j]);  $num_wt_2kb ++}
					}
					
				#	$mC_ros3_total = $mC_ros3_1kb + $mC_ros3_2kb;
				#	$mC_col0_total = $mC_col0_1kb + $mC_col0_2kb;
					
					$hyper_1kb = isHyper ($mC_ros3_1kb, $mC_col0_1kb, $min_coverage, $num_mut_1kb);
					$hyper_2kb = isHyper ($mC_ros3_2kb, $mC_col0_2kb, $min_coverage, $num_mut_2kb);
			#		$hyper_total = isHyper ($mC_ros3_total, $mC_col0_total , $min_coverage);
					
				
		
				print OUT join("\t", ($this, $num_mut_1kb, $num_wt_1kb, $mC_ros3_1kb, $mC_col0_1kb, $hyper_1kb, $num_mut_2kb, $num_wt_2kb, $mC_ros3_2kb, $mC_col0_2kb, $hyper_2kb)), "\n";
			}
...
sub isHyper{
	...
	if($wt == 0){
		if ($cov >= $cutoff and $mut >= 10){
			return "YES";
		}else{
			return "NO";
		}
	}else{
		if($cov >= $cutoff and $mut/$wt >= 2 and $mut >= 10){
			return "YES";
		}
		else{
			return  "NO";
