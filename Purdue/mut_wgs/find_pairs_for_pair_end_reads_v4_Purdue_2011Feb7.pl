...
my $out1 = Bio::SeqIO->new(-file => ">$output_pair1",-format=>'fastq',-width=> 300);
my $out2 = Bio::SeqIO->new(-file => ">$output_pair2",-format=>'fastq',-width => 300);

my $sigle_out = Bio::SeqIO->new(-file => ">$SE_out",-format=>'fastq',-width=>300);
...
foreach my $key (keys %hash)
{
	$single1++;
	my $id1 = $key."/1";
#	my $seq_out = Bio::Seq->new(-id => $id1,-seq =>${$hash{$key}});
	${$hash{$key}}->id($id1);
