...
# < /Users/tang58/scripts_all/perl_code/Purdue/Histone_src/histone_score/Nov21_read_density_RPKM/draw_DMRs_histone_RPKM_ratio_one_fig.r
...
################
setwd(indir)
#/Volumes/Macintosh_HD_2/Kai_project/cross_talk_of_RdDM_and_methylation/Jacobsen_PNAS_method_v2/downstream/connect_with_histone/IDM1_paper_list/ReadsN_as_PLOS_method/step1_read_num_outdir
files = dir(pattern= list_pre )
files_tmp = vector()

#print(files)

input_file = files[grep ("Input", files)]
#print("Input")
#print (input_file)

j = 0
for(i_tmp in 1:length(files)){
	if(files[i_tmp]!= input_file){
		j=j+1
		files_tmp[j] = files[i_tmp]
	}
}
...
input_data = read.delim( input_file , head=T, sep = "\t") 
...
input_RPKM = input_data[,3:(2*half_bin_num + 3)] * 10^9 / (binSize * h[["Input"]])
input_mean = colMeans(input_RPKM)
...
for (i in 1:file_num){ 
	...
	colmean = colMeans(list_RPKM_frame)
	list_RPKM_mean[[i]] = colmean/input_mean
...
plot(	
		...
		ylab = "ratio of ChIP-seq read density (ChIP/Input)", #colnames(x)[i], 
