...
# /Users/tang58/scripts_all/perl_code/Purdue/Histone_src/histone_score/draw_DMRs_histone_ratio_5kb.r
...
for (i in 2:11) {
	plot (x[,1],x[,i]/x[,12], 
		  type="l",#b,o,l
		  col="blue", xaxt="n", las = las_val,xlab="", ylab = colnames(x)[i], cex.lab= cex_lab, cex.axis = cex_axis_plot, mgp = mgp) ; 
   #ylab="", xlab = colnames(x)[i])
	abline(v=mid_point, lty= "dotted") 
#axis(1, at = c(1,41,81), labels=c("-2kb","Midpoint","+2kb"), cex.axis=cex_axis)
axis(1, at = c(1,mid_point,2*mid_point - 1), labels=c("-5kb","Midpoint","+5kb"), cex.axis=cex_axis)
}
