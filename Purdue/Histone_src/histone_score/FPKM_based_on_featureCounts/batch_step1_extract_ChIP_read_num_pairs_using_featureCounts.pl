...
my $script = "/Users/tang58/scripts_all/perl_code/Purdue/Histone_src/histone_score/FPKM_based_on_featureCounts/step1_extract_ChIP_read_num_pairs_using_featureCounts.pl";
...
my $usage = "$0 \n <DMR_indir> <bam_dir> <outdir> <type: bed or coor> <CPU_thread>\n\n";
...
foreach my $file(@DMR_lists){
	if ( $file =~ /(\S+)\.txt$/) {
		...
		die unless (-e $input);
		my $cmd = "perl $script $input $type $bam_dir $outdir $pre $CPU_thread";
		print STDERR $cmd, "\n\n";
		if ( !$debug) {
			`$cmd`;#code
		}
